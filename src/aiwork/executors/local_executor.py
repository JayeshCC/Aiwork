        ...
        while attempt <= task.retries:
            try:
                ...
                if input_guardrails:
                    if verbose:
                        print(
                            f"    [Task {task.name}] Validating input with {len(input_guardrails)} guardrails..."
                        )
                ...
                if output_guardrails:
                    if verbose:
                        print(
                            f"    [Task {task.name}] Validating output with {len(output_guardrails)} guardrails..."
                        )
                ...
                metrics.record(
                    "task_duration_seconds", duration, {"task": task.name, "status": "success"}
                )
                ...
                if attempt > task.retries:
                    ...
                    duration = time.time() - start_time
                    metrics.record(
                        "task_duration_seconds", duration, {"task": task.name, "status": "failed"}
                    )
